...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, encoder_dim=512, dropout=0.5):
        ...
        #Visual Sentinel
        self.sentinel_linear = nn.Linear(decoder_dim + embed_dim, decoder_dim, bias=True)
        self.tanh = nn.Tanh()
        ...
        self.h_attn_fc = nn.Linear(decoder_dim + attention_dim, decoder_dim)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            ...
            sentinel_gate = self.sigmoid(self.sentinel_linear(torch.cat((h_previous[:batch_size_t], embeddings[:batch_size_t, t, :]),1)))
            visual_sentinel = sentinel_gate * self.tanh(c[:batch_size_t])
            visual_sentinel = visual_sentinel.unsqueeze(1)
            adaptive_attn_input = torch.cat((encoder_out[:batch_size_t], visual_sentinel),1)

            gate = self.sigmoid(self.f_beta(h[:batch_size_t]))  # gating scalar, (batch_size_t, encoder_dim)
            attention_weighted_encoding, alpha = self.attention(adaptive_attn_input,
                                                                h[:batch_size_t])
            attention_weighted_encoding = gate * attention_weighted_encoding

            h_attn_output = self.h_attn_fc(torch.cat((attention_weighted_encoding, h), 1))
            h_attn_output = self.tanh(h_attn_output)
            preds = self.fc(self.dropout(h_attn_output))  # (batch_size_t, vocab_size)
            predictions[:batch_size_t, t, :] = preds
            alphas[:batch_size_t, t, :] = alpha
         #此时alphas的最后一维向量即是来自语言模型的概率
#        print(alphas[0,:,-1])
